...
def train(x: List[GraphWrapper], y: Dict[str, Dict[Tuple[int, int], torch.Tensor]], config: Dict,
    ...
    combo: List[Tuple[int, int]] = get_combo(x[0].adj.shape[0], config["iterations"])
    for epoch in custom_range:
        for graph, c in zip(x * (len(combo) // len(x)), combo):
            optimizer.zero_grad()
    ...
    a = [
        reconstructed_matrix_to_shortest_path(predictor(prepare_input(c[0], c[1], x[0].adj.shape[0]), g.adj).data, c[0],
            ...
            y[str(g)][(c[0], c[1])], c[0]) for g, c in zip(x * (len(combo) // len(x)), combo)]
